...
tid = create_task(uid, pid, None, [uid], "", "", datetime.now() + timedelta(minutes=100), 2, "Low", "In Progress")
...
def test_get_user_workload():
    ...
    params = {'pid': pid}
    resp = requests.get(url + '/workload/get_user_workload', headers=headers_dict, params=params)

    assert resp.status_code == 200
    
def test_update_user_availability():
    headers_dict = {'Authorization': uid}
    json_dict = {'pid': pid, 'availability' : 4}
...
def test_workload_get_availability():
    headers_dict = {'Authorization': uid}
    params = {'pid': pid}
    resp = requests.get(url + '/workload/get_availability', headers=headers_dict, params=params)
    assert resp.status_code == 200
    
def test_workload_get_availability_ratio():
    headers_dict = {'Authorization': uid}
    params = {'pid': pid}
    resp = requests.get(url + '/workload/get_availability_ratio', headers=headers_dict, params=params)
    assert resp.status_code == 200
    
def test_workload_calculate_supply_demand():
    params = {'pid': pid}
    resp = requests.get(url + '/workload/calculate_supply_demand', params=params)
    assert resp.status_code == 200
    
def test_workload_get_supply_demand():
    params = {'pid': pid}
    resp = requests.get(url + '/workload/get_supply_demand', params=params)
    assert resp.status_code == 200
